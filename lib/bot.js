...
const { validateChannelMapping } = require('./validators');
const { formatFromDiscordToIRC, formatFromIRCToDiscord } = require('./formatting');
...
class Bot {
  constructor(options) {
    ...
    this.ircPreventMention = options.ircPreventMention;
    this.channels = _.values(options.channelMapping);
    this.ircStatusNotices = options.ircStatusNotices;
    this.announceSelfJoin = options.announceSelfJoin;
    this.webhookOptions = options.webhooks;
    this.webhookDisableEveryone = options.webhookDisableEveryone !== false; // default to true
  ...
  findDiscordChannel(ircChannel) {
    ...
    if (discordChannelName) {
      ...
      if (!discordChannel) {
        logger.info("Tried to send a message to a channel the bot isn't in: ", discordChannelName);
        return null;
      }
  ...
  sendToDiscord(author, channel, text) {
    ...
    const withMentions = withFormat
      .replace(/@([^\s#]+)#(\d+)/g, (match, username, discriminator) => {
        ...
        const user = guild.members.find(
          x => Bot.caseComp(x.user.username, username) && x.user.discriminator === discriminator
        );
        if (user) return user;
      ...
      .replace(/^([^@\s:,]+)[:,]|@([^\s]+)/g, (match, startRef, atRef) => {
        ...
        // eslint-disable-next-line
        const nickUser = guild.members.find(
          x => x.nickname !== null && Bot.caseComp(x.nickname, reference)
        );
